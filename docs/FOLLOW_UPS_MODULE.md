...
The manually applied extension is now used by the follow-up UI: creation and pending edits accept customer, company, or both; completion stores a separate customer response and can create a linked pending next follow-up. Completed records are read-only and overdue remains a derived display state.

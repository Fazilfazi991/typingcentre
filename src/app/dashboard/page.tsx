...
export default async function Dashboard({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const expiredRecords = portfolio.filter((record: any) => (daysFor(record) ?? 0) < 0);
  ...
  const cards = [
    ["Expired", expired, "alert", "danger", "Requires action", "View expired documents", renewalRangePath("expired"), `View ${expired} expired documents`],

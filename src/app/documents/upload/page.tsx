...
export default async function SmartDocumentUpload({ searchParams }: { searchParams: Promise<{ customerId?: string; companyId?: string; documentId?: string }> }) {
  ...
  const [customerResult, companyResult, documentResult, typesResult, customersResult, companiesResult] = await Promise.all([
    ...
    !customerId && !companyId && !documentId ? context.supabase.from("customers").select("id,full_name,companies(name)").eq("organization_id", context.organization.id).is("archived_at", null).order("full_name").limit(200) : Promise.resolve({ data: null }),
    !customerId && !companyId && !documentId ? context.supabase.from("companies").select("id,name").eq("organization_id", context.organization.id).is("archived_at", null).order("name").limit(200) : Promise.resolve({ data: null }),
  ]);
  ...
  if (!customerId && !companyId && !documentId) return <WorkspaceShell organizationName={context.organization.name} activePath="/documents"><header className="page-heading"><Link href="/documents">Back</Link><h1>Add document</h1><p>Choose an owner before uploading a scanned document.</p></header><DocumentOwnerPicker customers={(customersResult.data ?? []).map((customer: any) => ({ id: customer.id, name: customer.full_name, companyName: Array.isArray(customer.companies) ? customer.companies[0]?.name : customer.companies?.name }))} companies={(companiesResult.data ?? []).map((company: any) => ({ id: company.id, name: company.name }))} /></WorkspaceShell>;

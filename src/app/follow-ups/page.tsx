...
export default async function FollowUps({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <WorkspaceShell organizationName={context.organization.name} activePath="/follow-ups">
    ...
    <section className="panel follow-up-schedule-panel"><h2>{dateFilter ? "Scheduled today" : "Scheduled follow-ups"}</h2>{followUps?.length ? <FollowUpList followUps={followUps} destination={destination} now={now} timezone={context.organization.timezone}/> : <p className="empty-state">{dateFilter ? "No follow-ups scheduled for today." : "No follow-ups yet."}</p>}</section>
  </WorkspaceShell>;
}

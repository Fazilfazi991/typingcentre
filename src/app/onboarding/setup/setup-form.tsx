...
export function SetupForm({ organization }: { organization: { name: string; location: string; email: string; phone: string; address: string; step: number } }) {
  ...
  return <div className="onboarding-setup"><ol className="onboarding-progress" aria-label="Onboarding progress"><li className="complete">Workspace</li><li className={step === 2 ? "active" : "complete"}>Business details</li><li className={step === 3 ? "active" : ""}>Ready</li></ol>
    {step === 2 && <><div className="auth-intro"><p>Step 2 of 3</p><h1>Confirm your business details</h1><span>Only contact information is optional. Everything remains editable in Settings.</span></div><form action={action} className="auth-form"><input type="hidden" name="step" value="2"/><label>Typing centre name<input name="name" defaultValue={organization.name} required/></label><label>Emirate<select name="location" defaultValue={organization.location} required>{UAE_EMIRATES.map((emirate) => <option key={emirate}>{emirate}</option>)}</select></label><label>Business email <span>(optional)</span><input name="email" type="email" defaultValue={organization.email}/></label><label>Business phone <span>(optional)</span><input name="phone" inputMode="tel" defaultValue={organization.phone}/></label><label>Address <span>(optional)</span><input name="address" defaultValue={organization.address}/></label><button className="auth-submit" disabled={pending}>{pending ? "Saving..." : "Save and continue"}</button></form></>}

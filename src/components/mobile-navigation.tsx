...
export function MobileNavigation({ canImport, logoutAction }: { canImport: boolean; logoutAction: () => void | Promise<void> }) {
  ...
  return <>
    ...
    {open && <div className={styles.layer} role="presentation" onMouseDown={(event) => event.target === event.currentTarget && setOpen(false)}>
      <section className={styles.sheet} role="dialog" aria-modal="true" aria-labelledby="mobile-more-title">
        ...
        <form action={logoutAction}><button type="submit" className={styles.logout}>Log out</button></form>
      </section>
    </div>}
  </>;
}

...
export const onboardingSchema = z.object({ name: z.string().trim().min(2).max(160), location: z.enum(UAE_EMIRATES), displayName: z.string().trim().min(2).max(120), phone: z.string().trim().max(24).refine((value) => !value || value.length >= 7, "Enter a valid phone number."), acceptTerms: z.literal(true) });

...
export function buildExpiryEmail(input: {
  ...
  const section = (title: string, documents: ExpiryEmailDocument[]) =>
    documents.length
      ...
      : "";
  const html = `<!doctype html><html><body style="margin:0;background:#f7f9fc;font-family:Arial,sans-serif;color:#021952"><main style="max-width:620px;margin:24px auto;background:#fff;padding:32px;border-radius:10px"><div style="font-size:20px;font-weight:700;color:#021952">Note It</div><h1 style="font-size:24px;margin:24px 0 8px">Expiry summary</h1><p style="margin:0;color:#667085">${escapeHtml(input.organizationName)}</p><p style="line-height:1.5">You have <strong>${count}</strong> document${count === 1 ? "" : "s"} requiring attention.</p><table role="presentation" width="100%" style="background:#edf5ff;border-radius:8px;padding:12px"><tr><td>Today<br><strong>${input.digest.today.length}</strong></td><td>Next 7 days<br><strong>${input.digest.next7Days.length}</strong></td><td>Next 30 days<br><strong>${input.digest.next30Days.length}</strong></td></tr></table>${section("EXPIRING TODAY", input.digest.today)}${section("NEXT 7 DAYS", input.digest.next7Days)}${section("NEXT 30 DAYS", input.digest.next30Days)}<p style="margin:32px 0 8px"><a href="${escapeHtml(input.dashboardUrl)}" style="display:inline-block;background:#0e7bff;color:#fff;text-decoration:none;padding:12px 18px;border-radius:6px;font-weight:700">View Expiring Documents</a></p><p style="font-size:12px;color:#667085">Automated expiry notification from Note It.</p></main></body></html>`;
  return { subject: `Note It: ${count} document${count === 1 ? "" : "s"} need attention`, html };
}
